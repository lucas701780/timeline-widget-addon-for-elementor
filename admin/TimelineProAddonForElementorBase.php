...
if ( ! class_exists( 'TimelineProAddonForElementorBase' ) ) {
	class TimelineProAddonForElementorBase {
		public $key              = '3CEF30318CFC816B';
		private $product_id      = '10';
		private $product_base    = 'twae';
						...
						add_filter(
							...
							function( $links, $plugin_file ) {
								if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
									$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=twae_fupc' ) . "'>Update Check</a>";
								}
								return $links;
							},
							10,
		...
		function initActionHandler() {
			...
			if ( isset( $_GET['action'] ) && ( ( htmlspecialchars( $_GET['action'], ENT_QUOTES ) ) == $handler ) ) {
				$this->handleServerRequest();
				exit;
			}
		}
		function handleServerRequest() {
			$type = isset( $_GET['type'] ) ? strtolower( htmlspecialchars( $_GET['type'], ENT_QUOTES ) ) : '';
			switch ( $type ) {
