 ...
 *****************************************************************************************************
 */
if ( ! class_exists( 'TWAE_PRO_admin_notices' ) ) :
	...
	final class TWAE_PRO_admin_notices {
		...
		public function twae_pro_add_message( $notice ) {
			...
			add_action( 'admin_notices', array( $this, 'twae_pro_show_notice' ) );
			add_action( 'admin_print_scripts', array( $this, 'twae_pro_load_script' ) );
			add_action( 'wp_ajax_twae_pro_admin_notice', array( $this, 'twae_pro_admin_notice_dismiss' ) );
			add_action( 'wp_ajax_twae_pro_admin_review_notice_dismiss', array( $this, 'twae_pro_admin_review_notice_dismiss' ) );
		}
		 ...
		 * @return void
		 */
		public function twae_pro_load_script() {
			wp_register_style( 'twae-feedback-notice-styles', TWAE_PRO_URL . 'assets/css/twae-admin-feedback-notice.css', array(), TWAE_PRO_VERSION, 'all' );
			...
			if ( count( $this->messages ) > 0 ) {
				foreach ( $this->messages as $id => $message ) {
					if ( true == (bool) $message['review'] ) {
						...
						$this->twae_pro_simple_notice( $id, $message );
					}
				}
			}
		}
		 ...
		 */
		public function twae_pro_admin_review_notice_dismiss() {
...
function twae_pro_create_admin_notice( $notice ) {
	...
	$main_class = TWAE_PRO_admin_notices::twae_pro_create_notice();
	$main_class->twae_pro_add_message( $notice );
	return $main_class;
}
